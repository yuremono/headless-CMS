/**
 * セクション型ごとの簡易 markup 生成（examples/preview/js/sections.js の TS 移植）。
 * CMS は表示責任を持たないため、プレビュー／静的エクスポート専用。
 */
...
function readImage(value: unknown): ImageRef | null {
...
function readButton(value: unknown): ButtonRef | null {
...
function renderImage(image: ImageRef | null, className: string): string {
  if (!image) {
    return "";
  }
...
function renderButton(button: ButtonRef | null, className: string): string {
  if (!button) {
    return "";
  }
...
function renderTitleBlock(title: string, lead: string): string {
  const parts: string[] = [];
  if (title) {
...
function renderFallback(section: Record<string, unknown>): string {
  return `<pre class="preview_section__fallback">${escapeHtml(JSON.stringify(section, null, 2))}</pre>`;
}

function wrapSection(type: string, id: string, innerHtml: string, extraClass = ""): string {
...
function renderHero(data: Record<string, unknown>): string {
...
function renderTitleGroup(data: Record<string, unknown>): string {
  const title = readText(data.title);
...
function renderImageText(data: Record<string, unknown>): string {
  const title = readText(data.title);
  const body = readText(data.body) || readText(data.content);
  const image = readImage(data.image);
  const position = readText(data.imagePosition).toLowerCase();
  const layoutClass =
    position === "left" ? "is_image_left" : position === "right" ? "is_image_right" : "";
...
function renderCardList(data: Record<string, unknown>): string {
  const title = readText(data.title);
...
function renderFeatureList(data: Record<string, unknown>): string {
  const title = readText(data.title);
...
function renderFaq(data: Record<string, unknown>): string {
  const title = readText(data.title);
...
function renderCta(data: Record<string, unknown>): string {
  const title = readText(data.title);
...
function renderNewsList(data: Record<string, unknown>): string {
  const title = readText(data.title);
...
function renderGallery(data: Record<string, unknown>): string {
  const title = readText(data.title);
...
function renderCompanyProfile(data: Record<string, unknown>): string {
...
function renderAccess(data: Record<string, unknown>): string {
...
function renderContactLead(data: Record<string, unknown>): string {
  const title = readText(data.title);
  const lead = readText(data.lead) || readText(data.body);
  const button = readButton(data.button);
...
const SECTION_RENDERERS: Record<string, (data: Record<string, unknown>) => string> = {
  hero: renderHero,
...
export function renderPageHero(hero: unknown): string {
...
export function renderSections(sections: unknown): string {
