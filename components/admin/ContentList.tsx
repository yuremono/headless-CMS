		...
		<section className="ContentList space-y-4  border border-white/10 bg-white/5 p-5 shadow-xl shadow-slate-950/20">
			<div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
				...
				<div className="flex flex-wrap gap-3">
					{!readOnly ? (
						<Link
							href={`/sites/${siteId}/contents/${contentType.slug}/new`}
			...
			{current ? (
				<article className="rounded-md border border-white/10 bg-slate-950/60 p-5">
					<div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
						...
						<div className="flex flex-wrap gap-3">
							<Link
								href={`/sites/${siteId}/contents/${contentType.slug}/${current.id}`}
		...
		<section className="ContentList space-y-4  border border-white/10 bg-white/5 p-5 shadow-xl shadow-slate-950/20">
			<div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
				<div>
					...
					<p className="mt-2 text-sm text-slate-300">
						キーワード検索、ステータス絞り込み、削除、公開をここで操作できます。
					</p>
				</div>
				<div className="flex flex-wrap gap-3">
					{!readOnly ? (
							...
							<tr key={item.id}>
								...
								<td className="px-4 py-4">
									<div className="flex flex-wrap gap-2">
										<Link
											href={`/sites/${siteId}/contents/${contentType.slug}/${item.id}`}
