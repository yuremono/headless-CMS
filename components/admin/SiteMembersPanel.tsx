  ...
  return (
    <section className="SiteMembersPanel rounded-2xl border border-white/10 bg-slate-950/40 p-4">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-white">サイトメンバー</h2>
          <p className="text-sm text-slate-400">メールアドレスで招待し、ロールを割り当てます。</p>
        </div>
        <p className="text-xs text-slate-500">{members.length} 名</p>
      </div>

      {message ? <div className="mt-4"><AdminActionNotice kind={messageKind} message={message} /></div> : null}

      <form className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto_auto]" onSubmit={handleInvite}>
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[32rem] text-left text-sm">
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={4} className="px-2 py-6 text-slate-400">
                  読み込み中…
                </td>
              </tr>
            ) : members.length === 0 ? (
              ...
              members.map((member) => (
                <tr key={member.id} className="border-b border-white/5">
                  <td className="px-2 py-3 font-mono text-xs text-slate-200">{member.email ?? '—'}</td>
                  <td className="px-2 py-3 text-slate-300">{member.name ?? '—'}</td>
                  <td className="px-2 py-3">
                    <select
                      value={member.role}
                      onChange={(event) =>
                        void handleRoleChange(member, event.target.value as (typeof ROLE_OPTIONS)[number])
                      }
