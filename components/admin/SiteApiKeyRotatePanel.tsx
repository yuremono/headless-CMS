...
export function SiteApiKeyRotatePanel({ siteId, siteName }: SiteApiKeyRotatePanelProps) {
  ...
  if (rotatedKeys) {
    return (
      <section className="SiteApiKeyRotatePanel rounded-2xl border border-amber-400/20 bg-amber-400/5 p-4">
        <AdminActionNotice
          kind="success"
        ...
        <dl className="mt-4 grid gap-3 text-sm">
          <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-3">
            ...
            <dd className="mt-1 break-all font-mono text-amber-100">{rotatedKeys.apiKeys.public}</dd>
          </div>
          <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-3">
            <dt className="text-xs uppercase tracking-[0.2em] text-slate-500">管理 API キー</dt>
            <dd className="mt-1 break-all font-mono text-amber-100">{rotatedKeys.apiKeys.admin}</dd>
          </div>
        </dl>
        <button
          type="button"
  ...
  return (
    <section className="SiteApiKeyRotatePanel rounded-2xl border border-white/10 bg-slate-950/50 p-4">
      <h3 className="text-base font-semibold text-white">API キー</h3>
      <p className="mt-2 text-sm leading-6 text-slate-400">
        公開・管理キーをまとめて再発行します。再発行後は旧キーで API にアクセスできません。
      </p>
      <button
        type="button"
