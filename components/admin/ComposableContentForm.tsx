...
export interface ComposableFieldDefinition {
  prefix: string;
  repeatable?: boolean;
  fields: Array<Pick<ComposableFieldRow, 'type' | 'suffix' | 'format' | 'bundle'>>;
}
...
function createDefaultDirectory(groups: ComposableFieldGroup[]): ComposableFieldDirectory {
  return {
...
function hydrateDirectories(
  ...
  const directories =
    input?.directories
      .map((directory) => ({
        ...
        prefixes: uniqueStrings(directory.prefixes).filter((prefix) => groupPrefixSet.has(prefix)),
      }))
  ...
  const assigned = new Set(directories.flatMap((directory) => directory.prefixes));
  const unassigned = groupPrefixes.filter((prefix) => !assigned.has(prefix));
  if (unassigned.length === 0) {
    return directories;
  }

  const defaultIndex = directories.findIndex((directory) => directory.id === DEFAULT_DIRECTORY_ID);
  if (defaultIndex === -1) {
    return [
      {
        id: DEFAULT_DIRECTORY_ID,
  ...
  return directories.map((directory, index) =>
    index === defaultIndex
      ? { ...directory, prefixes: uniqueStrings([...directory.prefixes, ...unassigned]) }
      : directory,
  );
}
...
function serializeDirectories(
  ...
  return {
    ...
    ...(serializedDirectories.some((directory) => directory.id === activeDirectoryId)
      ? { activeDirectoryId }
      : {}),
  };
}
  ...
  return definitions.groups
    .map((definition) => {
      ...
      return {
        ...
        prefix,
        fields,
        ...(definition.repeatable ? { repeatable: true as const, items: [] } : {}),
      };
    })
...
function mergeGroupsWithDefinitions(
  ...
  const byPrefix = new Map(dataGroups.map((group) => [getGroupPrefix(group), group]));
  for (const definitionGroup of definitionGroups) {
    ...
    const existingSuffixes = new Set(current.fields.map((field) => field.suffix));
    const missingFields = definitionGroup.fields.filter((field) => !existingSuffixes.has(field.suffix));
    if (missingFields.length > 0) {
      byPrefix.set(prefix, { ...current, fields: [...current.fields, ...missingFields] });
    }
  }

  return Array.from(byPrefix.values()).sort((left, right) => left.prefix.localeCompare(right.prefix));
}

function collectComposableFieldDefinitions(groups: ComposableFieldGroup[]): ComposableFieldDefinitions {
  return {
    groups: groups
      .map((group) => ({
...
export function ComposableContentForm({
  ...
  const activeDirectory = directories.find((directory) => directory.id === activeDirectoryId) ?? directories[0];
  const visiblePrefixSet = new Set(activeDirectory?.prefixes ?? []);
  const visibleGroups = groups.filter((group) => visiblePrefixSet.has(getGroupPrefix(group)));
    ...
    setDirectories((current) =>
      current.map((directory) =>
        directory.id === activeDirectoryId
          ? { ...directory, prefixes: uniqueStrings([...directory.prefixes, prefix]) }
          : directory,
      ),
    );
  }
  ...
  function updateGroup(groupId: string, nextGroup: ComposableFieldGroup) {
    ...
    const previousPrefix = previousGroup ? getGroupPrefix(previousGroup) : '';
      ...
      setDirectories((directoryList) =>
        directoryList.map((directory) => ({
          ...
          prefixes: uniqueStrings(
            directory.prefixes.map((prefix) => (prefix === previousPrefix ? nextPrefix : prefix)),
          ),
        })),
      );
    }
  ...
  function removeGroup(groupId: string) {
    ...
    if (removedPrefix) {
      setDirectories((directoryList) =>
        directoryList.map((directory) => ({
          ...directory,
          prefixes: directory.prefixes.filter((prefix) => prefix !== removedPrefix),
        })),
      );
    }
  }
  ...
  function handleDuplicateGroup(groupId: string) {
    ...
    setDirectories((directoryList) =>
      directoryList.map((directory) => {
        ...
        const sourcePrefixIndex = directory.prefixes.indexOf(source.prefix);
  ...
  function handleRenameDirectory(directoryId: string, name: string) {
    setDirectories((current) =>
      current.map((directory) => (directory.id === directoryId ? { ...directory, name } : directory)),
    );
  }
  ...
  function handleConfirmDeleteDirectory() {
    ...
    setDirectories((current) => {
      ...
      const removedDirectory = current.find((directory) => directory.id === removedId);
      const remainingDirectories = current.filter((directory) => directory.id !== removedId);
      ...
      return remainingDirectories.map((directory, index) =>
        index === 0
          ? { ...directory, prefixes: uniqueStrings([...directory.prefixes, ...movedPrefixes]) }
          : directory,
      );
    });
    ...
    setActiveDirectoryId((currentId) => {
      ...
      const nextDirectory = directories.find((directory) => directory.id !== removedId);
  ...
  async function persist(action: 'save' | 'publish') {
    ...
    const body = buildContentWriteBody({
      ...
      fieldDirectories: serializeDirectories(directories, groups, activeDirectoryId),
  ...
  return (
    <section data-l="ContentForm" className="ComposableContentForm flex h-full flex-col gap-4 overflow-visible lg:flex-row lg:gap-6">

        <aside data-l="DirectoryAside" className="DirectoryAside lg:w-64 lg:shrink-0 ">
          <nav className="AdminNav flex min-h-full flex-col gap-6">
            <div data-l="NavBrand">
              <h1 className="pt-1 text-xl font-medium text-SC tracking-wider">
                Modular <span className="text-SC [font-size:1em]">Headless</span> CMS
                <span className="block text-GR [font-size:0.75em]">inspired by microCMS.</span>
              </h1>
                      ...
                      <div
                        ...
                        onKeyDown={(event) => {
                          if (event.key === 'Enter' || event.key === ' ') {
                            ...
                            setActiveDirectoryId(directory.id);
                          }
                        }}
                      >
                          ...
                          <div className="min-w-0 flex-1">
                            <span className="group relative block">
                              <input
                                ...
                                onChange={(event) => handleRenameDirectory(directory.id, event.target.value)}
                        ...
                        {isActive && directories.length > 1 ? (
                          <button
                            ...
                            onClick={(event) => {
                              ...
                              setPendingDirectoryDelete(directory);
                            }}
        ...
        <div className=" mt-0 flex min-w-0 flex-1 flex-col items-stretch gap-4  lg:flex-row lg:items-start lg:gap-6 border border-TC/20 p-4 lg:p-6 lg:overflow-y-auto ">
          <FieldAddPanel
            ...
            actionNotice={<AdminActionNotice kind={statusKind} message={statusMessage} />}
          />

          <div className="min-w-0 flex-1 h-full overflow-y-auto pr-2">
            {groups.length === 0 ? (
        ...
        {pendingDirectoryDelete ? (
          ...
          >
            <div className="w-full max-w-sm rounded-md border border-TC/20 bg-WH p-5 shadow-xl">
              <h2 id="DirectoryDeleteDialogTitle" className="text-base font-bold text-TC">
                ディレクトリを削除しますか？
              </h2>
              <p className="mt-2 text-sm text-GR">
                {pendingDirectoryDelete.name} を削除します。中のフィールドは先頭のディレクトリへ移動します。
              </p>
