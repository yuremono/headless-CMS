...
export function SiteCreateForm() {
  ...
  if (createdSite) {
    ...
    return (
		<section className="SiteCreateForm rounded-md border border-emerald-400/20 bg-emerald-400/5 p-4">
			<AdminActionNotice
				kind="success"
				message={`「${createdSite.site.name}」を作成しました。API キーはこの画面でのみ表示されます。`}
			/>
  ...
  return (
		<form
			className="SiteCreateForm rounded-md border border-white/10 bg-slate-950/50 p-4"
		...
		>
			...
			<p className="mt-3 text-xs leading-5 text-slate-500">
				コンテンツ種類 JSON を取り込み、公開・管理 API
				キーを発行します。スラッグは URL と API パスで使います。
			</p>
