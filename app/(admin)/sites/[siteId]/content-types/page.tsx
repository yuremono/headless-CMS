...
export default async function ContentTypesPage({ params }: ContentTypesPageProps) {
  ...
  return (
		<AdminLayout site={site}>
			...
			/>
			<AdminApiNotice
				source={contentTypes.meta.source}
				error={contentTypes.meta.error}
				endpoint={contentTypes.meta.endpoint}
			/>
			<div className=" border border-white/10 bg-white/5 p-5 text-sm text-slate-300">
				管理画面は GUI 必須です。JSON
				定義だけではなく、一覧と編集の両方からスキーマを確認できます。
			</div>
			<ContentTypeList
				siteId={siteId}
